...
class GoogleAnalyticsAPI
{
     ...
     *
     */
    public function getVisitsByYearMonth($params = array())
    {
    ...
    public function getVisitsByYear($params = array())
    {
    ...
    public function getVisitsByCountryRegion($params = array())
    {
    ...
    protected function _query($params = array())
    {
        ...
        $ret = json_decode($data, $this->assoc);
        $message = date('Y-m-d h:i:s') . ' GoogleAnalyticsAPI: ' . print_r($queryParams, true) . "\nReturns:\n" . print_r($ret, true);
//echo '<pre>'.$message;
        if (!empty($this->queryLog)) {

            error_log($message, 3, $this->queryLog);
        }
        return $ret;
    }
